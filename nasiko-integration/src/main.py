...
from router.src.core.load_balancer import HealthCheckerDaemon, LoadBalancer
...
# Initialize orchestrator, health daemon, and load balancer
health_daemon = HealthCheckerDaemon()
load_balancer = LoadBalancer(health_daemon)
orchestrator = RouterOrchestrator(health_daemon=health_daemon, load_balancer=load_balancer)
...
async def process_request(
    ...
    strategy: str = Form("round-robin"),  # Load balancing strategy
    ...
    try:
        ...
        return StreamingResponse(
            orchestrator.process_request(request, files_to_forward, token, strategy),
...
async def startup_event():
    """Start the health checker daemon on app startup."""
    import asyncio
    asyncio.create_task(health_daemon.run())


@app.on_event("shutdown")
async def shutdown_event():
    """Stop the health checker daemon on app shutdown."""
    health_daemon.stop()
