...
class RouterOrchestrator:
    ...
    def __init__(self, health_daemon=None, load_balancer=None):
        self.agent_registry = AgentRegistry(health_daemon=health_daemon)
    ...
    async def process_request(
        ...
        try:
            ...
            async for response in self._handle_route_selection(request, files, token, strategy):
                yield response
    ...
    async def _handle_route_selection(
        ...
        # Step 6: Get agent URLs and select instance using load balancer
        try:
            agent_urls = self.agent_registry.get_agent_urls(agent_name)
            if not agent_urls:
            ...
            if self.load_balancer:
                selected_url, routing_reason = await self.load_balancer.select_instance(
                    agent_name, agent_urls, strategy
                )
                logger.info(f"Load balancer selected: {routing_reason}")
            else:
                # Fall back to first URL if no load balancer
                selected_url = agent_urls[0]
                routing_reason = f"Fallback to first URL: {selected_url}"
            
            # Send request to selected agent
            async for response in self._send_agent_request(
                request, files, selected_url, token, agent_name
            ):
                yield response
    ...
    async def _send_agent_request(
        ...
        try:
            ...
            if self.load_balancer:
                await self.load_balancer.increment_connections(agent_url)
            ...
            if self.load_balancer:
                self.load_balancer.increment_request_count(agent_url)
            ...
            if self.load_balancer:
                self.load_balancer.increment_error_count(agent_url)
            yield self._router_response(str(e), "", False, agent_url)
        finally:
            # Decrement active connections
            if self.load_balancer:
                await self.load_balancer.decrement_connections(agent_url)
