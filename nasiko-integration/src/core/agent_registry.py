...
class AgentRegistry:
    ...
    def __init__(self, health_daemon=None):
        self.timeout = httpx.Timeout(settings.REQUEST_TIMEOUT)
        self._cache: Optional[List[Dict[str, str]]] = None
        self._cache_timestamp: Optional[float] = None
        self.health_daemon = health_daemon
        self.agent_urls: Dict[str, List[str]] = {}  # key: agent_name, value: list of urls
    ...
    async def fetch_agent_cards(
        ...
        try:
            async with httpx.AsyncClient(timeout=self.timeout) as client:
                ...
                # Extract and register URLs with health daemon
                self.agent_urls.clear()
                for agent_card in agent_cards:
                    agent_name = agent_card.get("name")
                    urls = []
                    # Check for "urls" field first (list of URLs)
                    if "urls" in agent_card and isinstance(agent_card["urls"], list):
                        urls = agent_card["urls"]
                    # Fall back to "url" field (single URL)
                    elif "url" in agent_card:
                        urls = [agent_card["url"]]
                    # Register each URL
                    for url in urls:
                        if url:
                            if self.health_daemon:
                                self.health_daemon.register_url(url)
                            if agent_name:
                                if agent_name not in self.agent_urls:
                                    self.agent_urls[agent_name] = []
                                if url not in self.agent_urls[agent_name]:
                                    self.agent_urls[agent_name].append(url)
    ...
    def get_agent_urls(self, agent_name: str) -> List[str]:
        """
        Get all URLs for a specific agent.

        Args:
            agent_name: Name of the agent

        Returns:
            List of URLs for the agent
        """
